import React, { useState, useEffect } from "react";
import { useLocation } from "react-router-dom";
import Button from "./Button";
import { brainwave } from "../assets";
import MenuSvg from "../assets/svg/MenuSvg";
import { navigation } from "../constants";
import { HambugerMenu } from "./design/Header";
import { disablePageScroll, addScrollableTarget } from "scroll-lock";

const Header = () => {
  const pathname = useLocation();
  const [openNavigation, setOpenNavigation] = useState(false);

  useEffect(() => {
    const selectedSection = document.querySelector(pathname.hash);
    addScrollableTarget(selectedSection);
  }, [pathname]);

  useEffect(() => {
    if (openNavigation) disablePageScroll();
  }, [openNavigation]);

  const toggleNavigation = () => {
    setOpenNavigation(!openNavigation);
  };

  const handleClick = () => {
    setOpenNavigation(false);
  };

  return (
    <div
      className={`${
        openNavigation ? `bg-n-8` : "bg-n-8/90 backdrop-blur-sm"
      }  fixed top-0 left-0 w-full z-50 bg-n-8/90 
       border-b border-n-6 lg:bg-n-8/90 
      lg:backdrop-blur-sm`}
    >
      <div className="flex items-center  px-5 lg:px-7.5 xl:px-10 max-lg:py-4">
        <a className="block w-[12rem] xl:mr-8 " href="#hero">
          <img src={brainwave} width={190} height={40} alt="Brainwave" />
        </a>

        <nav
          className={` fixed top-[5rem] left-0 right-0 bottom-0
         bg-n-8 lg:static lg:flex lg:mx-auto lg:bg-transparent
         ${openNavigation ? `flex` : `hidden`}`}
        >
          <div className="relative z-2 flex flex-col items-center justify-center m-auto lg:flex-row">
            {navigation.map(({ id, title, url, onlyMobile }) => {
              return (
                <a
                  href={url}
                  key={id}
                  onClick={handleClick}
                  className={`block relative font-code text-2xl uppercase 
                  text-n-1 transition-colors hover:text-color-1 ${
                    onlyMobile ? "lg:hidden" : ""
                  } lg:leading-5 lg:hover:text-n-1 xl:px-12 px-6 py-6 md:py-8 lg:mr-0.25 lg:text-xs lg:font-semibol ${
                    pathname.hash === url ? `z-2 lg:text-n-1 ` : `text-n-1/50`
                  }`}
                >
                  {title}
                </a>
              );
            })}
          </div>
          <HambugerMenu />
        </nav>

        <a
          href="#signup"
          className="button hidden text-n-1/50 transition-colors mr-8
           hover:text-n-1 lg:block "
        >
          New account
        </a>
        <Button className="hidden lg:flex" href="#login">
          Sign in
        </Button>

        <Button
          className={`ml-auto lg:hidden`}
          px="px-3"
          onClick={toggleNavigation}
        >
          <MenuSvg openNavigation={openNavigation} />
        </Button>
      </div>
    </div>
  );
};

export default Header;
